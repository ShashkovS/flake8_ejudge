...
Кроме того, модуль позволяет выполнить проверку используемого синтаксиса.
...
Для каждой проверки требуется *все* три переменных: ``chk_pattern_i``, ``chk_in_or_not_i`` и ``chk_err_msg_i``.
Число i должно быть от 1 до 30 без ведущих нулей. Допускаются пропуски индексов.

В переменной ``chk_pattern_i`` должно быть указано регулярное выражение, которое будет искаться при помощи модуля ``re``
по всему коду. При этом символ ``\`` нужно дополнительно экранировать, поэтому если нужно найти все слова ``and``, то следует писать ``style_checker_env = "chk_pattern_1=\\band\\b"```.

Используйте модификаторы ``(?m)`` для поднятия флага ``re.MULTILINE``, ``(?s)`` для поднятия флага
